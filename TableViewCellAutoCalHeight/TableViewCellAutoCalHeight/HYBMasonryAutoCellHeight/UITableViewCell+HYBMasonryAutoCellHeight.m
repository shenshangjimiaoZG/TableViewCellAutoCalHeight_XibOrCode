...
#pragma mark - Public
+ (CGFloat)hyb_heightForTableView:(UITableView *)tableView cellType:(CellType)type config:(HYBCellBlock)config {
  ...
  cellType:(CellType)type
    ...
    if (cache) {
        ...
        if (tableView.hyb_cacheCellHeightDict.count == 0
            ...
            || cacheHeight == nil) {
            CGFloat height = [self hyb_heightForTableView:tableView cellType:type config:config];
    ...
    return [self hyb_heightForTableView:tableView cellType:type config:config];
}
